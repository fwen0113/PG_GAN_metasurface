...
class Generator(nn.Module):
    ...
    def forward(self, noise, labels, params):
        ...
        y0 = net
        print('before_conv_size1',net.size())
        ...
        print('before_conv_size',net.size())
        
        print('scheme_index',self.scheme_index)
        ...
        print('after_conv_size',net.size())
        print('y0_size',y0.size())
...
class Discriminator(nn.Module):
    ...
    def forward(self, img, labels,params):
        ...
        print('scheme is',self.scheme_index)
        if(self.scheme_index is not 0):
            print('y0 size',y0.size())
            print('from2d',self.from2d)
            net = self.blocks[params.scheme_index-1](net)
            print('net size',net.size())
            print('after',F.adaptive_avg_pool2d(y0,net[0][0].size()))
        ...
        for block_index in range(params.scheme_index-2,-1,-1):
            ...
            print('test',self.blocks[block_index])
        #    if(i == self.scheme_index-2):
        #        y0 = net
        ...
        print('after conv',net.size())
        net = net.view(net.size(0), -1)

        net = torch.cat([net, labels], -1)
        print('net final',net.size())
        net = self.FC(net)
